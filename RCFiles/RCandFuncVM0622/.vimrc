...
"if exists('+colorcolumn') 
    "set colorcolumn=80 " Color column settings (cc can be used for short)
"else
...
" Notes: (these usually used inside vim, not in vimrc)
"     :set fileformat=unix " or set ff=dos to control what it writes as
"         (This is local to each buffer!)
"     :set fileformats=unix,dos " specifies the formats to be read
"         (This is global, and the first listed is set as default for new
"         buffers.  Shortcut is ffs, as in :set ffs?)
"     :verbose set ff? ffs? " will display where last set.
"     :e ++ff=dos   " Edit the file again, using dos file format
"         ('fileformats' is ignored)
"     :setlocal ff=unix   " This buffer only use LF-only endings when written
"     :w ++ff=dos         " Write the current file as dos(can also use unix)
"     
"     A nice little way to change the fileformat for multiple files:
"         vim +"argdo :w ++ff=dos" +q *.cpp *.h
"         With this, the +q stands for quit.  Can also use things like +wqa
"             which is the same as :xa
"
"
"     
"
